        ...
        active_page  (nullptr        )
{
        ...
        for (unsigned int i = 0; i < p_pages; ++i) {
                h = new FilledWindow(
                ...
                h->set_callback_leftclick(
                        [&active_page, &pages, i](HUDElement * self) {
                                active_page = pages[i];
                        }
                );
...
void Tabs::draw(GLuint active_hud_elem) const {
        ...
        glPushMatrix();
                ...
                if (active_page) active_page->draw(active_hud_elem);
                ...
                for (int i = 0; i < n_pages; ++i) {
                        glPushMatrix();
                                glTranslatef(
                                        ((2*i + 1)*header_width - width)/2.0,
                                        (header_height + height)/2.0,
                                        0
                                );
